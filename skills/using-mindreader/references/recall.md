...
- Both accept `detail`: `detailed` is the default full envelope; `concise` returns handles plus thin subject/predicate/object lines and clears `about`. Compact around witness paths remain available in both modes. Concise `iris` keeps top-level `facts[]` empty.
- `iris` accepts 1–20 unique node IRIs, preserves input order, and reports misses and per-lookup truncation through `lookups[i]`. `hops` is `0` or `1` and defaults to `0`; incident fact handles still appear on `lookups[i].facts`. `limit` is per requested IRI. `hops: 1` also fills top-level `facts[]` when `detail` is `detailed`.
- `around` requires a node IRI. Its `p` filter, `direction` (`both`|`outgoing`|`incoming`, default `both`), and `depth` (`1..=3`, default `1`) apply only to this selector and constrain every traversed edge. `paths[i]` is the deterministic hub-aware witness path for `facts[i]`, with compact `{iri,from,p,to}` edges.
- `history` accepts one node or fact IRI and returns current and `validTo` facts plus newest-first exact `revisions[]` events. Use their previous/replacement fact handles normally; `supersedes` is audit metadata and is never pasteable.
