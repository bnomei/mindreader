...
| Filter explicitly time-qualified states to one world-time instant | Either recall tool with `effectiveAt`; never use it merely because an event question mentions a date |
...
- `effectiveAt` is a timezone-qualified RFC 3339 instant accepted by text, non-catalog labels, `iris`, `around`, and semantic recall. It returns only transaction-current ordinary facts whose explicit half-open effective interval contains the instant. Unknown-time facts are excluded. Structural edges may remain in an IRI or neighborhood path, but every returned ordinary fact must pass the filter.
- `history` accepts one node or fact IRI and returns current and `validTo` facts plus newest-first exact `revisions[]` events. It rejects `effectiveAt`. History facts expose `transactionCurrent`, `transaction:{from,to}`, and their independent `effective` interval. In detailed mode, use previous/replacement fact handles normally; `supersedes` is audit metadata and is never pasteable.
...
## Retrieve comparisons without hiding evidence

For ordering, counting, duration, and other comparisons, retrieve every named anchor and the dates or quantities needed to compare them. Start with stable entity and action terms rather than a full question. Do not add an `around.p` filter until a result or catalog lookup establishes the stored predicate vocabulary; related sessions may use several valid predicates for the same broader concept.

If the first lexical query misses an expected anchor, retry once with its concrete entity terms, an inflection or close synonym, or an unfiltered `around` walk from a known node. Use semantic recall only when the remaining gap is conceptual and provider disclosure is acceptable. A matching endpoint or nearby entity is not evidence for the requested relationship; inspect the returned predicate and object before answering.

Missing evidence stays unknown. In particular, a fact without temporal qualification is not earlier or later than a dated fact. Do not use `effectiveAt` to find point events, dated purchases, attendance, or facts whose effective time may be unknown: retrieve those normally and compare their explicit event-date facts afterward.
...
Effective filtering applies after lexical or activation candidates are grounded, including structural expansion. An old activation can therefore recall only facts still current, visible, and effective at the requested instant. Do not add `effectiveAt` merely because a question mentions a date: point-event dates are ordinary facts on event nodes and need ordinary lexical/graph recall. Use the filter for state-as-of questions.
