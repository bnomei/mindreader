...
- Tighten mutation and recall schemas against common model call errors: omit selector-only and kind-specific defaults, validate IRI-shaped inputs and non-empty edits earlier, reject `withdraw.p` with an exact target, expose persisted literal node handles needed for endpoint-closed placement, and serialize explicitly qualified open effective intervals as `{}` instead of schema-invalid null bounds.
